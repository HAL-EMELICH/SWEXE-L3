...
Process.fork do
  ...
  def upload(d)
    u = (!ENV['DEVMODE']) ? 'https://dmss-bxvo.onrender.com/upload' : 'http://localhost:3030/upload'
    uri = URI.parse(u)
  ...
  while true
    ...
    if s.include?( 'Error' )
      ...
      puts (!ENV['DEVMODE']) ? "https://dmss-bxvo.onrender.com/?uid=#{uid}" : "http://localhost:3030/?uid=#{uid}"
      next
    end
